...
Add-Content $logFilePath "`n Script Name: ClonePermissionsBP1DomainUsers"
		...
		if($SourceUserPermissions)
		{
			if($SourceRoleAssignment.Member -is [Microsoft.SharePoint.SPGroup])   
			{
				...
				if( ($Group.Users | where {$_.UserLogin -eq $TargetUserID}) -eq $null )
				{
				  ...
				  Add-Content $logFilePath "`n Added to Group: $($Group.Name)"
			...
			Add-Content $logFilePath "`n ***$($ObjectType) Permissions Copied: $($SourceUserPermissions) at $($ObjectURL) via $($SourcePermissionType)***"
...
try
{
    ...
    {
		Add-Content $logFilePath "`n Scanning $site..."
		...
		Add-Content $logFilePath "`n Scanning Site Collection Administrators Group for:" $site.Url
		...
		foreach($SiteCollAdmin in $Site.RootWeb.SiteAdministrators)
		{
			if($SiteCollAdmin.LoginName.EndsWith($SourceUserID,1))
			{
				...
				$TargetUser.Update()
				Add-Content $logFilePath "`n ***Added to Site Collection Admin Group***"
			}     
		}
		...
		foreach($Web in $WebsCollection)
		{
			...
			Add-Content $logFilePath "`n Scanning Lists on $($web.url)..."
	...
	Add-Content $logFilePath "`n Permission are copied successfully!"
...
$SourceUser = "c:0-.t|bp-id-provider|BP1\domain users"
$TargetUser = "c:0!.s|trusted%3abp-id-provider"
$OutputReport = "BP1OutputReport.txt"
